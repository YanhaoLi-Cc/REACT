    ...
    for batch_idx in batch_iter:
        try:
            ...
            for entry in batch_entries:
                ...
                batch_original_data.append(entry)
            ...
            for i, output_ids in enumerate(batch_output_ids):
                ...
                # 构建DPO格式数据
                dpo_entry = {
                    "id": original_data.get("id", i),
                    "molecules": original_data.get("molecules", {}),
                    "chosen_messages": original_data.get("messages", []),  # 原始对话作为正样本
                    "rejected_messages": [  # 构建负样本对话
                        msg for msg in original_data.get("messages", [])[:-1]  # 复制除最后一条外的消息
                    ] + [{
                ...
                references.append(original_data.get("ground_truth", ""))
            ...
            if args.cache_dir and current_index % 5 == 0:
                ...
                # 保存DPO数据
                with open(os.path.join(args.cache_dir, "dpo_dataset.json"), "w", encoding='utf-8') as f:
